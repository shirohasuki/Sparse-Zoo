...
class Network(nn.Module):
   ...
   #def save_tensors_to_csv(self, sparse_attention, v, B, N, T, D):
       
        sparse_attention_np = sparse_attention.detach().cpu().numpy()
        v_np = v.detach().cpu().numpy()

        
        for i in range(B):
            df_sparse_attention = pd.DataFrame(sparse_attention_np[i])
            df_sparse_attention.to_csv(f'sparse_attention_batch_{i}.csv', index=False)

       
        for i in range(B):
            df_v = pd.DataFrame(v_np[i].reshape(N, T * D))
            df_v.to_csv(f'v_batch_{i}.csv', index=False)